...
function santasNaughtyList(persons, badHabit) {
    ...
    for(let person in persons){
        if(persons[person].habits.includes(badHabit)){
            santasNaughtyList.push(persons[person].firstName + " " + persons[person].lastName)
        }
    }
...
function santasNaughtyListFunctional(persons, badHabit) {
    return persons.filter((person) => person.habits.filter((h) => h === badHabit ).length > 0 ).map((person) => person.firstName + " " + person.lastName)
